...
- `CTRL+n` - Complete word (Press CTRL+n again to cycle between suggestions)
- `:set complete=` - Choose where autocompletion comes from
- `:sbuffer {number}` - Create split using buffer number x (:ls to see buffers)
...
By default `:set command=.,w,b,u,t,i`
